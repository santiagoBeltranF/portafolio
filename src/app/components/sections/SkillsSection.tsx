...
import { ShieldCheck, Code, Server, Brain } from "lucide-react";
    ...
    <TooltipProvider delayDuration={100}>
      <Tooltip>
        ...
        <TooltipContent side="top" className="font-pixel bg-card border-primary text-primary-foreground">
          ...
          <p className="text-xs text-primary-foreground/90">Nivel: {skill.levelText} ({skill.dataLevel}/100)</p>
          {skill.description && <p className="text-xs text-primary-foreground/70 mt-1 max-w-[200px]">{skill.description}</p>}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
...
const SkillBranch: React.FC<SkillBranchProps> = ({ title, skills, icon }) => (
  <Card className="skill-branch p-4 sm:p-6 pixel-corners border-2 border-primary/50 shadow-xl bg-background/70 backdrop-blur-sm">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
      {skills.map((skill) => (
        <SkillNode key={skill.title} skill={skill} />
      ))}
    </div>
  </Card>
);
